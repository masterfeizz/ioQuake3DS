...
{
	int haddr = htonl(((struct sockaddr_in *)input)->sin_addr.s_addr);

	sprintf(dest, "%d.%d.%d.%d:%d", (haddr >> 24) & 0xff, (haddr >> 16) & 0xff, (haddr >> 8) & 0xff, haddr & 0xff, ntohs(((struct sockaddr_in *)input)->sin_port));
